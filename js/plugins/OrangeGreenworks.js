...
function store (str){
  ...
  var path = require('path');
    ...
    var rePath = path.resolve(base, '..');

    fs.appendFile(base + '/' +'_log.txt' , str + "\n", function(err){
      if (err) {
            console.error(err);
       }
  });
...
var chimaki_alias_plugin_command = Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    chimaki_alias_plugin_command.call(this, command, args);
    if (command == "ACH"){
        switch (args[0]){
            case "ADD" :
                OrangeGreenworks.activateAchievement(args[1]);
                break;
            case "DEL" :
                OrangeGreenworks.clearAchievement(args[1]);
                break;
            case "GET_LAN":
